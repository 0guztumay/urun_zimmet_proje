...
<body>
    <div class="container w-25">
        <h2 class="text-center" style="margin-top: 30%">Add Brand</h2>
        <form action="{{route("add-brand")}}" method="POST" style="margin-top: 10%">
            @csrf
                ...
                @foreach ($brands as $brand)
                    <li class="list-group-item d-flex justify-content-between align-items-center text-center">
                        <form action="{{route("update-brand", ["id" => $brand -> id])}}" method="POST">
                            @csrf
                            <input type="text" name="name" class="form-control" style="margin-left: 30%" placeholder="Brand Name" value="{{$brand -> name}}"/>
                            
                            <div style="margin-left: 30%">
                                <button class="btn btn-primary btn-sm me-2" data-bs-toggle="modal" data-bs-target="#editModal" type="submit">Update</button>
                                <a href="{{route("delete-brand", ["id" => $brand -> id])}}" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteModal">Delete</a>
                            </div>
                        </form>
                    </li>
                @endforeach
